...
function TypeScriptToXS(filename: string, isInclude: boolean = false): void {
    ...
    if(!IGNORE_ERRORS) errors.forEach(async e => {
        ...
        while(match != null) {
            ...
            console.log(`${' '.repeat(4-line.toString().length)}\x1b[1m${line}\x1b[0m | ${match[0].replace(e.highlight, t => "\x1b[31m" + t + "\x1b[0m")}
...
<!-- Original script: \`${filename + '\`' + ' '.repeat(40 - filename.length)} -->
<!-- Conversion date: \`${DATE + '\`' + ' '.repeat(40 - DATE.length)} -->
